...
timelineContainer.addEventListener("mousedown" , toogleScrubbing)
...
let isSrcubbing = false
let wasPaused
function toogleScrubbing(e){
      const rect = timelineContainer.getBoundingClientRect();
      const percent =
        Math.min(Math.max(0, e.x - rect.x), rect.width) / rect.width;
        isSrcubbing = (e.buttons & 1) === 1
        videoContainer.classList.toggle("scrubbing" , isSrcubbing)
        if (isSrcubbing) {
